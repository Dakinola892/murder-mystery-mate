<template>
    <div class="ui container">
        ...
        <div class="ui large feed container">
            <component v-for="event in events" :key="event.id" :is="eventType(event.type)" :event="event" @showEditModal="showEditModalMethod($event)"></component>
        </div>
        <modal-new-event @closeModal="showModal = false"  :open="showModal"></modal-new-event>
        <modal-edit-event @closeModal="showEditModal = false" :open="showEditModal" :event-prop="selectedEvent"></modal-edit-event>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            currentPage: 0,
            events: this.$store.getters.timelineEvents
        }
    },
    components: {
        ModalNewEvent,
